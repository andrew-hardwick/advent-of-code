...
def get_ops():
	return {
		'AND': lambda children, nodes: lookup_or_parse(children[0], nodes) & lookup_or_parse(children[1], nodes),
		'OR': lambda children, nodes: lookup_or_parse(children[0], nodes) | lookup_or_parse(children[1], nodes),
		'LSHIFT': lambda children, nodes: lookup_or_parse(children[0], nodes) << lookup_or_parse(children[1], nodes),
		'RSHIFT': lambda children, nodes: lookup_or_parse(children[0], nodes) >> lookup_or_parse(children[1], nodes),
		'NOT': lambda children, nodes: ~lookup_or_parse(children[0], nodes),
		'MONAD': lambda children, nodes: lookup_or_parse(children[0], nodes),
	}
...
def parse_input(infn):
	...
	with open(infn, 'r') as f:
		node_data = [parse_node(l, ops) for l in f.readlines()]
