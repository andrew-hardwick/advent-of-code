...
def execute(infn, width, target_rock_count):
	...
	print(math.lcm(len(jets), len(rocks)))

	simulation_length = math.lcm(len(jets), len(rocks))

	partial = target_rock_count % simulation_length

	multiple = int(target_rock_count / simulation_length)

	for count in range(simulation_length):
		process_turn(board, rocks, jets, count)

		if count == partial:
			partial_height = len(board) - find_highest_rock(board) - 1

	full_height = len(board) - find_highest_rock(board) - 1

	return full_height * multiple + partial
...
if __name__ == '__main__':
	main('test1.txt', 7, 1000000000000)
	main('input.txt', 7, 1000000000000)